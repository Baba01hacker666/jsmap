...
def substep(m):  print(f"  {C.BG_BLU}{C.WHITE} ► {C.RESET} {C.BOLD}{m}{C.RESET}")
...
class OutputLayout:
    ...
    def create_all(self):
        for d in [
            self.chunks_dir, self.maps_dir, self.sources_dir,
            self.ng_project_dir, self.ng_src_dir, self.ng_dist_dir,
...
class ChunkDownloader:
    ...
    def __init__(self, session: requests.Session, base_url: str,
                 layout: OutputLayout, threads: int = 5, delay: float = 0.0):
    ...
    def download_chunk(self, chunk_id: str, chunk_hash: str,
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                self._dl_log.append({"file": filename, "url": url, "bytes": sz, "status": "ok"})
                ...
                if alt:
                    ...
                    sz = len(alt.content)
                    self._dl_log.append({"file": filename, "url": url, "bytes": sz, "status": "alt"})
                    return {"status": "ok", "filename": filename, "size": sz, "note": "alternate_path"}
                error(f"  404: {filename}")
                self._dl_log.append({"file": filename, "url": url, "status": "404"})
                return {"status": "404", "filename": filename}
            else:
                self._dl_log.append({"file": filename, "url": url, "status": f"http_{resp.status_code}"})
    ...
    def auto_detect_chunks(self) -> Tuple[dict, dict, list]:
        ...
        try:
            ...
            runtime_url = next(
                (u for u in all_scripts if re.search(r'runtime', u, re.IGNORECASE)), None
            )
            if not runtime_url:
    ...
    def download_all(self, chunk_map: dict = None, special_names: dict = None,
        ...
        if chunk_map:
            substep(f"Downloading {len(chunk_map)} chunks  [threads={self.threads}]")
        ...
        if extra_scripts:
            substep(f"Downloading {len(extra_scripts)} detected script references")
        ...
        log_path.write_text(json.dumps(self._dl_log, indent=2), encoding="utf-8")
...
class ExternalToolExtractor(BaseExtractor):
    ...
    def is_available(self) -> bool:
        if self._available is None:
            try:
                subprocess.run([self.tool_name, "--version"], capture_output=True, check=True)
...
class NativeRegexExtractor(BaseExtractor):
    ...
    RULES = {
        ...
        "secrets": [
            ...
            {"name": "JWT Token",          "severity": "CRITICAL",
            ...
            {"name": "Slack Webhook",      "severity": "HIGH",
             "patterns": [r'(https://hooks\.slack\.com/services/T[A-Z0-9]+/B[A-Z0-9]+/[A-Za-z0-9]+)']},
            {"name": "GitHub Token",       "severity": "CRITICAL",
             "patterns": [r'(ghp_[A-Za-z0-9]{36}|github_pat_[A-Za-z0-9_]{82})']},
            {
                "name": "Generic Secret",
                "severity": "HIGH",
                "patterns": [
        ...
        "config": [
            ...
            {"name": "Database Connection String",
             "severity": "CRITICAL",
             "patterns": [r'["\`]((?:mongodb|postgres|mysql|redis|mssql)://[^\s"\'`]{10,250})["\`]']},
            {"name": "S3 Bucket",
             "severity": "HIGH",
             "patterns": [
        ...
        "debug": [
            {"name": "Console Log Leak",
             "severity": "LOW",
             "patterns": [r'console\.(?:log|warn|error)\s*\([^)]{0,30}(?:password|token|secret|key)[^)]{0,100}\)']},
            {"name": "Debug Flag",
             "severity": "MEDIUM",
             "patterns": [r'(?:debug|debugMode|DEV_MODE|disableAuth|enableDevTools)\s*[=:]\s*true']},
            {"name": "Admin Panel Path",
             "severity": "HIGH",
             "patterns": [r'["\`](/(?:admin|administrator|manage|superadmin|control-panel|swagger|api-docs|dashboard)[/a-zA-Z0-9_\-]*)["\`]']},
                    ...
                    try:
                        for m in re.finditer(pattern, content, re.MULTILINE | re.DOTALL):
                            ...
                            findings.append(Finding(
                                ...
                                value=value[:600], severity=severity,
                                file=filepath.name, line=line_no,
                                context=context[:350], tool=self.name,
                            ))
                    except re.error:
                        continue
        return findings
...
class RipgrepExtractor(ExternalToolExtractor):
    ...
    def _build_command(self):
        ...
        if self.patterns_file:
            ...
            cmd.extend([
                "-e", r"(api|v\d|graphql|rest)/",
                "-e", r"AKIA[0-9A-Z]{16}",
                "-e", r"AIza[0-9A-Za-z\-_]{35}",
            ])
        cmd.append("{file}")
        return cmd
        ...
        for line in output.strip().split("\n"):
            try:
                ...
                if data.get("type") == "match":
                    ...
                    findings.append(Finding(
                        category="pattern_match", subcategory="ripgrep_hit",
                        value=text[:300], severity="INFO",
                        file=filepath.name, line=match["line_number"],
                        tool="ripgrep",
                    ))
            except Exception:
                continue
...
class SourceMapReconstructor:
    ...
    def extract_strings(self, chunks_dir: Path) -> Dict:
        ...
        success(f"Strings dump: {len(urls)} URLs · {len(paths)} paths · {len(emails)} emails → reports/all_strings.json")
        return result
            ...
            "jsmap-recon": {
                ...
                "architect": {
                    ...
                    "serve": {
                        ...
                        "configurations": {
                            "production":  {"buildTarget": "jsmap-recon:build:production"},
                            "development": {"buildTarget": "jsmap-recon:build:development"},
                        },
...
"""
    ...
    def __init__(self, layout: OutputLayout, ng_version: str = None):
        self.layout     = layout
    ...
    def _check_prerequisites(self) -> bool:
        for tool in ["node", "npm", "ng"]:
            if not shutil.which(tool):
                error(f"'{tool}' not found in PATH. Install Node.js + Angular CLI first.")
    ...
    def scaffold(self, extracted_sources: Optional[Path] = None) -> bool:
        ...
        (src / "index.html").write_text(self.MINIMAL_INDEX_HTML, encoding="utf-8")
        (src / "styles.css").write_text("/* global styles */\n", encoding="utf-8")
        (src / "favicon.ico").write_bytes(b"")
        (app / "app.module.ts").write_text(self.MINIMAL_APP_MODULE,     encoding="utf-8")
        (app / "app.component.ts").write_text(self.MINIMAL_APP_COMPONENT, encoding="utf-8")
        ...
        if extracted_sources and extracted_sources.exists():
            ...
            shutil.copytree(str(extracted_sources), str(dest_recon), dirs_exist_ok=False)
            success(f"Sources copied → ng_project/src/recon/")
    ...
    def build(self) -> bool:
        ...
        try:
            ...
            # Append to build log
            existing = self.build_log.read_text(encoding="utf-8") if self.build_log.exists() else ""
            ...
            success(f"Build complete!  {len(js_files)} JS bundle(s) in ng_project/dist/")

            # Copy dist to layout for easy access
            dest_dist = self.layout.ng_dist_dir
            if dest_dist.exists():
                shutil.rmtree(dest_dist)
            shutil.copytree(str(self.project / "dist"), str(dest_dist), dirs_exist_ok=False)
            success(f"Dist output → {dest_dist.relative_to(self.layout.root)}/")
            return True
...
def write_summary(layout: OutputLayout, target_url: str, findings: List[Finding],
                  dl_stats: dict, build_ok: Optional[bool]):
    summary = {
        "target":       target_url,
        ...
        "ng_build":     ("success" if build_ok else "failed") if build_ok is not None else "skipped",
        "layout": {
    ...
    layout.summary_file.write_text(json.dumps(summary, indent=2), encoding="utf-8")
...
""",
    ...
    parser.add_argument("--analyze-only",   action="store_true", help="Phase 2 only (needs --dir)")
    ...
    an.add_argument("--severity",        default="INFO",
    ...
    ng.add_argument("--ng-build",    action="store_true",
                    help="Phase 4: scaffold + ng build --configuration production")
    ng.add_argument("--ng-version",  default=None, help="Override Angular version (default: ^17.0.0)")
    ng.add_argument("--ng-only",     action="store_true",
    ...
    if not args.analyze_only:
        ...
        downloader = ChunkDownloader(session, args.url, layout, args.threads, args.delay)
        ...
        downloader = type("FakeDownloader", (), {"stats": {"ok":0,"skipped":0,"failed":0}})()
    ...
    if args.extract_sources or args.ng_build:
        ...
        if src_dir:
            ...
            if new_count:
                success(f"{new_count} additional findings in reconstructed sources")
    ...
    write_summary(layout, args.url or args.dir or "", findings, downloader.stats, build_ok)

    # ── Final summary ─────────────────────────────────────────────────────────
    print(f"\n{C.BOLD}{'═'*68}{C.RESET}")
    print(f"{C.BOLD}  COMPLETE{C.RESET}  ·  output: {C.CYAN}{layout.root.resolve()}{C.RESET}")
    ...
    if build_ok is not None:
        status = f"{C.GREEN}SUCCESS{C.RESET}" if build_ok else f"{C.RED}FAILED{C.RESET}"
